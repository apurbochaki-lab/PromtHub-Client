...
import { authHeaderClient } from '@/lib/core/Token/getTokenClient';

const AddPromptFormClient = ({ user, myPostCount }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        // const authHeaders = await authHeaderClient();
        // const postedData = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/my-prompts?creatorId=${creatorId}`, {
        //     headers: {
        //         'Content-Type': 'application/json',
        //         ...authHeaders
        //     },
        // });
        // const postData = await postedData.json();
        ...
        // console.log("Total posted count : ", freeLimit.usedPost);
    ...
    if (user?.plan === "free") {
        if (myPostCount >= freeLimit.maxLimit) {
            return <div>Your limit finished</div>
        }
    }

    return (
        <div className="max-w-4xl mx-auto py-8 px-4">
            <div className="mb-10 border-b border-[#72b01d]/20 pb-6 flex items-center justify-between">
                <div>
                ...
                <div>
                    <h2>You posted {myPostCount} prompt out of {freeLimit.maxLimit}</h2>
                </div>
            </div>
